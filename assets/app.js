    ...
    if (!scrolling) {
      ...
      if (vPanelIndex >= 0 && vPanelIndex <= vMaxCount) {
        ...
        if (vPanelIndex == vMaxCount) {
          $(".nav").addClass('--hidden'); // hide nav when at footer
        ...
        vCount = vPanelIndex;
        // change desktop nav highlight
        $('.nav__item').removeClass('--active');
        if (vCount > 0 && vCount < vMaxCount) {
          $('.nav__item').eq(vCount - 1).addClass('--active');
        }
        // change mobile nav highlight
        $('.mobile-menu .panel__text').removeClass('--active');
        if (vCount > 0 && vCount < vMaxCount) {
          $('.mobile-menu .panel__text').eq(vCount - 1).addClass('--active');
        }
